...
def repeats(our_str):
    """
    Создаем строку, которая показывает колличество вхождений каждого символа, в формате _n
    :param our_str: строка в которой будут подсчитываться символы
    """
    new_list = []
    for i in range(len(our_str)):
        new_list.append('{}_{}'.format(our_str[i], our_str[0: i + 1].count(our_str[i])))
